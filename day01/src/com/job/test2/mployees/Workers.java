...
public class Workers extends Employees{
    ...
    public Workers(String name, double salary,String date,double percentage) {
        super(name,salary,date);
        this.percentage = (percentage/100);
    }
    @Override
    void showInfo() {
        System.out.println("工人"+getName()+"的基本信息为：");
        System.out.println("薪水:"+getSalary()+"\n雇用日期："+getDate());
        System.out.println("工资增长比:"+((Math.round(percentage*100))+"%")+"\n涨幅后的工资"+addAalary(percentage));
    }
    /**
     * 方法作用:计算涨的工资
     * 涨工资：原有工资+(原有工资乘与工资百分比)
     * */
    @Override
    public double addAalary(double percentage) {

        return Math.round(getSalary()+(getSalary()*percentage));
    }
}
